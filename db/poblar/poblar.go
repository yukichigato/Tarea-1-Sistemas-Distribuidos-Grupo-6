...
import (
	...
	"github.com/yukichigato/Tarea-1-Sistemas-Distribuidos-Grupo-6/db/bd"
)

func main() {
	db := bd.InitDB("mi_base.db")
	defer db.Close()

	fmt.Println("Poblando la base de datos con datos de ejemplo...")

	// Inserta un usuario de ejemplo
	_, err := db.Exec(`INSERT INTO users (first_name, last_name, email, password, usm_pesos) VALUES (?, ?, ?, ?, ?);`,
	...
	_, err = db.Exec(`INSERT INTO books (book_name, book_category, transaction_type, price, status, popularity_score) VALUES (?, ?, ?, ?, ?, ?);`,
		"El principito", "Infantil", "Venta", 10, "Disponible", 4)
	if err != nil {
		log.Fatalf("Error al insertar libro 1: %v", err)
	}

	_, err = db.Exec(`INSERT INTO inventory (id, available_quantity) VALUES (?, ?);`, 1, 6)
	if err != nil {
	...
	_, err = db.Exec(`INSERT INTO inventory (id, available_quantity) VALUES (?, ?);`, 2, 11)
	if err != nil {
	...
	_, err = db.Exec(`INSERT INTO inventory (id, available_quantity) VALUES (?, ?);`, 3, 1)
	if err != nil {
	...
	_, err = db.Exec(`INSERT INTO inventory (id, available_quantity) VALUES (?, ?);`, 4, 1)
	if err != nil {
	...
	_, err = db.Exec(`INSERT INTO inventory (id, available_quantity) VALUES (?, ?);`, 5, 0)
	if err != nil {
		log.Fatalf("Error al insertar inventario para libro agotado: %v", err)
	}

	// Inserta datos de préstamos de ejemplo
	_, err = db.Exec(`INSERT INTO loans (user_id, book_id, start_date, return_date, status) VALUES (?, ?, ?, ?, ?);`,
	...
	// Inserta datos de ventas de ejemplo
	_, err = db.Exec(`INSERT INTO sales (user_id, book_id, sale_date) VALUES (?, ?, ?);`,
		1, 1, "05/08/2025")
	if err != nil {
